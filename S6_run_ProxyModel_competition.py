...
finalStep = 100
variable_parameters = {"competition": np.linspace(0.1, 0.9, 3) , "seed": np.linspace(1, 9, 3) }
...
def making_DB(L):
    ...
    for k in range(len(L)):
        for key in store_dict.keys():
            store_dict[key].append(L[k][key])

    return pd.DataFrame.from_dict(store_dict)
